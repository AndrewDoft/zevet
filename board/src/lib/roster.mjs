/** Pure roster, follow-mode and turn-summary logic for the board. Plain
 *  JavaScript on purpose: the gate tests execute this exact file, and the
 *  React components import it, so the behaviour under test is the behaviour
 *  that ships. Everything here is a calculation — no DOM, no store — except
 *  the static agent marks, which are data. */

export function clampPaneWidth(v, min, max) {
  v = Number(v);
  if (!isFinite(v)) return min;
  return Math.max(min, Math.min(max, Math.round(v)));
}

export function newestHunk(hunks) {
  let best = null;
  (hunks || []).forEach((h) => {
    if (h && typeof h.start === "number" && (!best || h.start > best.start)) best = h;
  });
  return best;
}

export function followAllows(mode, actor, myActor) {
  if (mode === "off") return false;
  if (mode === "all") return true;
  return Boolean(myActor && actor === myActor);
}

export function lastToolFor(events, repoName, relPath) {
  const evts = events || [];
  for (let i = evts.length - 1; i >= 0; i--) {
    const ev = evts[i];
    if (ev && ev.kind === "tool" && ev.repo === repoName && ev.target === relPath) return ev;
  }
  return null;
}

/**
 * `lastToolFor`, but for every touched path at once — the map the file-tree
 * sprite needs. Built once per render (the tree can run to thousands of
 * rows), so this walks `events` a single time rather than having each row
 * call `lastToolFor` for its own path.
 *
 * Reuses `followAllows` rather than re-deriving "off shows nothing / mine
 * shows only me" here, so the tree's follow-mode gate cannot drift from the
 * one the editor rider already applies.
 *
 * ⚠️ LATEST WINS BY TIMESTAMP, not by array position. `events` is normally
 * append-ordered, but comparing `ev.ts` directly means a caller that hands in
 * an unordered slice still gets the right answer instead of a silently wrong
 * one.
 */
export function spritesByPath(events, { repoName, followMode, myActor, now, idleAfterMs }) {
  const out = {};
  (events || []).forEach((ev) => {
    if (!ev || ev.kind !== "tool" || !ev.target) return;
    if (repoName && ev.repo !== repoName) return;
    if (!followAllows(followMode, ev.actor, myActor)) return;
    if (now - ev.ts > idleAfterMs) return;
    const prev = out[ev.target];
    if (!prev || ev.ts >= prev.ts) out[ev.target] = { actor: ev.actor, tool: ev.tool, ts: ev.ts };
  });
  return out;
}

/** Everything of an actor's since their most recent prompt. */
export function turnTrace(events, actor) {
  const mine = (events || []).filter((e) => e && e.actor === actor);
  let startIdx = 0;
  for (let i = mine.length - 1; i >= 0; i--) {
    if (mine[i].kind === "prompt") {
      startIdx = i;
      break;
    }
  }
  const turn = mine.slice(startIdx);
  return {
    prompt: turn[0] && turn[0].kind === "prompt" ? turn[0] : null,
    tools: turn.filter((e) => e.kind === "tool"),
    ended: turn.some((e) => e.kind === "turn_end"),
  };
}

/** The one-line mission and the current command, as the roster shows them. */
export function turnSummary(entry, events) {
  const t = turnTrace(events, entry && entry.actor);
  const mission = t.prompt && t.prompt.detail ? String(t.prompt.detail).slice(0, 80) : "";
  const e = t.tools[t.tools.length - 1];
  const current = e
    ? (e.tool || "tool") + (e.target || e.detail ? "  " + (e.target || e.detail) : "")
    : t.ended ? "turn finished" : "";
  return { mission, current };
}

export function verbFor(e) {
  if (!e) return "working";
  if (e.kind === "prompt") return "asked";
  if (e.kind === "turn_end") return "finished";
  return e.tool || "working";
}

/** The folder a teammate is working in — the thing you want at a glance. */
export function folderOf(e) {
  if (!e || !e.target) return e && e.detail ? String(e.detail).slice(0, 40) : "";
  const parts = String(e.target).split("/");
  parts.pop();
  return parts.length ? parts.join("/") + "/" : "./";
}

export function ago(ms) {
  const s = Math.max(0, Math.round(ms / 1000));
  if (s < 60) return s + "s";
  const m = Math.round(s / 60);
  if (m < 60) return m + "m";
  return Math.round(m / 60) + "h";
}

export function agoText(now, ts) {
  return ago(now - ts) + " ago";
}

const DAY_MS = 86400000;

/** Real per-day event counts, oldest to newest, for a heat-map style widget.
 *  `days` is a display window, not a claim about how much history exists —
 *  the hub only ever keeps a rolling few hundred events (see hub/server.mjs),
 *  so a quiet day is legitimately 0 rather than omitted. Nothing here is
 *  invented: a day with no events in it reads as a day with no events in it. */
export function dailyActivity(events, days, now) {
  const span = Math.max(1, Math.floor(days) || 14);
  const end = Math.floor((now == null ? Date.now() : now) / DAY_MS);
  const start = end - (span - 1);
  const counts = new Map();
  (events || []).forEach((e) => {
    if (!e || typeof e.ts !== "number") return;
    const day = Math.floor(e.ts / DAY_MS);
    if (day < start || day > end) return;
    counts.set(day, (counts.get(day) || 0) + 1);
  });
  const out = [];
  for (let day = start; day <= end; day++) {
    out.push({ date: new Date(day * DAY_MS).toISOString().slice(0, 10), count: counts.get(day) || 0 });
  }
  return out;
}

/** The active collaborators whose last event is in a repo — one dot each. */
export function liveActorsOf(roster, repoName) {
  if (!repoName) return [];
  return (roster || []).filter((r) => r && r.lastEvent && r.lastEvent.repo === repoName);
}

/* Agent marks. Filed here as data rather than in the component so the gate can
 * check the coverage without importing React: known agents get a mark, and any
 * other agent name must fall through to the plain bead in marks.tsx. */
export const AGENT_MARKS = {
  "claude-code":
    '<svg class="fmark" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="m4.7144 15.9555 4.7174-2.6471.079-.2307-.079-.1275h-.2307l-.7893-.0486-2.6956-.0729-2.3375-.0971-2.2646-.1214-.5707-.1215-.5343-.7042.0546-.3522.4797-.3218.686.0608 1.5179.1032 2.2767.1578 1.6514.0972 2.4468.255h.3886l.0546-.1579-.1336-.0971-.1032-.0972L6.973 9.8356l-2.55-1.6879-1.3356-.9714-.7225-.4918-.3643-.4614-.1578-1.0078.6557-.7225.8803.0607.2246.0607.8925.686 1.9064 1.4754 2.4893 1.8336.3643.3035.1457-.1032.0182-.0728-.164-.2733-1.3539-2.4467-1.445-2.4893-.6435-1.032-.17-.6194c-.0607-.255-.1032-.4674-.1032-.7285L6.287.1335 6.6997 0l.9957.1336.419.3642.6192 1.4147 1.0018 2.2282 1.5543 3.0296.4553.8985.2429.8318.091.255h.1579v-.1457l.1275-1.706.2368-2.0947.2307-2.6957.0789-.7589.3764-.9107.7468-.4918.5828.2793.4797.686-.0668.4433-.2853 1.8517-.5586 2.9021-.3643 1.9429h.2125l.2429-.2429.9835-1.3053 1.6514-2.0643.7286-.8196.85-.9046.5464-.4311h1.0321l.759 1.1293-.34 1.1657-1.0625 1.3478-.8804 1.1414-1.2628 1.7-.7893 1.36.0729.1093.1882-.0183 2.8535-.607 1.5421-.2794 1.8396-.3157.8318.3886.091.3946-.3278.8075-1.967.4857-2.3072.4614-3.4364.8136-.0425.0304.0486.0607 1.5482.1457.6618.0364h1.621l3.0175.2247.7892.522.4736.6376-.079.4857-1.2142.6193-1.6393-.3886-3.825-.9107-1.3113-.3279h-.1822v.1093l1.0929 1.0686 2.0035 1.8092 2.5075 2.3314.1275.5768-.3218.4554-.34-.0486-2.2039-1.6575-.85-.7468-1.9246-1.621h-.1275v.17l.4432.6496 2.3436 3.5214.1214 1.0807-.17.3521-.6071.2125-.6679-.1214-1.3721-1.9246L14.38 17.959l-1.1414-1.9428-.1397.079-.674 7.2552-.3156.3703-.7286.2793-.6071-.4614-.3218-.7468.3218-1.4753.3886-1.9246.3157-1.53.2853-1.9004.17-.6314-.0121-.0425-.1397.0182-1.4328 1.9672-2.1796 2.9446-1.7243 1.8456-.4128.164-.7164-.3704.0667-.6618.4008-.5889 2.386-3.0357 1.4389-1.882.929-1.0868-.0062-.1579h-.0546l-6.3385 4.1164-1.1293.1457-.4857-.4554.0608-.7467.2307-.2429 1.9064-1.3114Z"/></svg>',
  codex:
    '<svg class="fmark" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M22.2819 9.8211a5.9847 5.9847 0 0 0-.5157-4.9108 6.0462 6.0462 0 0 0-6.5098-2.9A6.0651 6.0651 0 0 0 4.9807 4.1818a5.9847 5.9847 0 0 0-3.9977 2.9 6.0462 6.0462 0 0 0 .7427 7.0966 5.98 5.98 0 0 0 .511 4.9107 6.051 6.051 0 0 0 6.5146 2.9001A5.9847 5.9847 0 0 0 13.2599 24a6.0557 6.0557 0 0 0 5.7718-4.2058 5.9894 5.9894 0 0 0 3.9977-2.9001 6.0557 6.0557 0 0 0-.7475-7.0729zm-9.022 12.6081a4.4755 4.4755 0 0 1-2.8764-1.0408l.1419-.0804 4.7783-2.7582a.7948.7948 0 0 0 .3927-.6813v-6.7369l2.02 1.1686a.071.071 0 0 1 .038.052v5.5826a4.504 4.504 0 0 1-4.4945 4.4944zm-9.6607-4.1254a4.4708 4.4708 0 0 1-.5346-3.0137l.142.0852 4.783 2.7582a.7712.7712 0 0 0 .7806 0l5.8428-3.3685v2.3324a.0804.0804 0 0 1-.0332.0615L9.74 19.9502a4.4992 4.4992 0 0 1-6.1408-1.6464zM2.3408 7.8956a4.485 4.485 0 0 1 2.3655-1.9728V11.6a.7664.7664 0 0 0 .3879.6765l5.8144 3.3543-2.0201 1.1685a.0757.0757 0 0 1-.071 0l-4.8303-2.7865A4.504 4.504 0 0 1 2.3408 7.872zm16.5963 3.8558L13.1038 8.364 15.1192 7.2a.0757.0757 0 0 1 .071 0l4.8303 2.7913a4.4944 4.4944 0 0 1-.6765 8.1042v-5.6772a.79.79 0 0 0-.407-.667zm2.0107-3.0231l-.142-.0852-4.7735-2.7818a.7759.7759 0 0 0-.7854 0L9.409 9.2297V6.8974a.0662.0662 0 0 1 .0284-.0615l4.8303-2.7866a4.4992 4.4992 0 0 1 6.6802 4.66zM8.3065 12.863l-2.02-1.1638a.0804.0804 0 0 1-.038-.0567V6.0742a4.4992 4.4992 0 0 1 7.3757-3.4537l-.142.0805L8.704 5.459a.7948.7948 0 0 0-.3927.6813zm1.0976-2.3654l2.602-1.4998 2.6069 1.4998v2.9994l-2.5974 1.4997-2.6067-1.4997Z"/></svg>',
  opencode:
    '<svg class="fmark" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M16.778 1.844v1.919q-.569-.026-1.138-.032-.708-.008-1.415.037c-1.93.126-4.023.728-6.149 2.237-2.911 2.066-2.731 1.95-4.14 2.75-.396.223-1.342.574-2.185.798-.841.225-1.753.333-1.751.333v4.229s.768.108 1.61.333c.842.224 1.789.575 2.185.799 1.41.798 1.228.683 4.14 2.75 2.126 1.509 4.22 2.11 6.148 2.236.88.058 1.716.041 2.555.005v1.918l7.222-4.168-7.222-4.17v2.176c-.86.038-1.611.065-2.278.021-1.364-.09-2.417-.357-3.979-1.465-2.244-1.593-2.866-2.027-3.68-2.508.889-.518 1.449-.906 3.822-2.59 1.56-1.109 2.614-1.377 3.978-1.466.667-.044 1.418-.017 2.278.02v2.176L24 6.014Z"/></svg>',
};